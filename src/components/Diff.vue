<template>
  <div>
    ...
    <div style="display: flex">
      <div style="margin: 0 0 0 100px; flex-grow: 1">
        <el-upload
          ...
          <div slot="tip" class="el-upload__tip">需要同时上传2张jpg/png文件</div>
        </el-upload>

        <el-button style="margin-down: 10px;" size="medium" type="warning" @click="startRun">start run</el-button>
        <el-tag style="margin-down: 10px;" size="medium">将文件拖到此处，或<em>点击上传</em>,需要同时上传2张jpg/png文件</el-tag>
      </div>
    </div>
    ...
    <div class="demo-image__placeholder">  <!--动态变化的-->
      <el-image
        class="table-td-thumb"
        :src="require('@/assets/img/diff.png')"
      ></el-image>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      start_run_pic_url: this.$deploy_url + 'diff/start_run',
    ...
    startRun: function () {
      ...
      if (this.fileList.length < 2) {
        ...
        if (!isLt5M) {
          ...
          formData.append('file0', this.fileList[0].raw)
          formData.append('file1', this.fileList[1].raw)
          // this.fileList.forEach(file => {
          //   formData.append('file', file.raw)  //因为要上传多个文件，所以需要遍历
